from sqlalchemy import create_engine, MetaData, Table, Column, Integer, String, DateTime, Text, Enum
...
class db_manager:
    ...
    def update_fruit(self, fruit_id, stock):
        try:
            stmt = update(fruit_table).where(fruit_table.c.id == fruit_id).values(stock=stock).returning(fruit_table)
            with self.engine.connect() as conn:
                result = conn.execute(stmt)
